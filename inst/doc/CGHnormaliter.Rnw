...
\vspace{0.5cm}
\noindent Next, we run the CGHnormaliter routine on the first three
chromosomes of the \begin{tt}Leukemia\end{tt} data:
<<label=removeMAplot, echo=FALSE, results=HIDE>>=
unlink("MAplot.pdf")
<<runCGHnormaliter>>=
...
\begin{figure}
\begin{center}
\resizebox{12.8cm}{!}{\includegraphics[page=2]{MAplot}}
\end{center}
\caption{MA-plot for the second leukemia sample before and after
normalization. Note that the normalization is based on the normals only
(represented by black dots in the bottom figure).}
\label{fig:ma-plot}
\end{figure}
...
\vspace{0.5cm}
The results can be visualized using the \begin{tt}plot\end{tt} function.
In Figure~\ref{fig:calls-plot} we plot the results for the second sample
in full resolution:
<<label=plotCommand, term=FALSE>>=
plot(result[,2], ylimit=c(-2,2), dotres=1)
@

\begin{figure}
\begin{center}
<<label=callFigure, fig=TRUE, eps=FALSE, echo=FALSE, results=HIDE>>=
<<plotCommand>>
@
\end{center}
\vspace*{-1.4cm}
...
\vspace{0.5cm}
Finally, the package provides the function
...
<<label=removeJunkFiles, echo=FALSE, results=HIDE>>=
unlink(c("Rplots.pdf", "normalized.txt", "segmented.txt", "called.txt"))
@
