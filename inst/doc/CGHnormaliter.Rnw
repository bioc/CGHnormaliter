...
<<runCGHnormaliter>>=
result <- CGHnormaliter(Leukemia, nchrom=3, cellularity=0.9)
@
...
\vspace{0.5cm}
\noindent Plotting the normalized log2 ratios in a density plot provides
another means (besides MA plots) to inspect whether the centralization has
been successful. Figure~\ref{fig:density-plot} shows such a density plot
for sample 2:
...
\noindent The results, including segments and calls, can be visualized using
the \begin{tt}plot\end{tt} function. In Figure~\ref{fig:calls-plot} we plot
the results for the second sample
in full resolution:
